odoo.define("apco.TemplateDialog", function (require) {
    "use strict";

    console.log('test apco templatedialog');
    ...
    class TemplateDialog extends owl.Component {
        ...
        async _createWord() {
            ...
            const name =
                templateId !== null
                    ? this.state.templates.find((template) => template.id === templateId).name
                    : UNTITLED_SPREADSHEET_NAME;
                const WordId = await this.env.services.rpc({
                    model: "documents.document",
                    method: "copy_word",
                    args: [
                        this.props.folderId,
                ...
                this.env.services.notification.notify({
                    type: "info",
                    message: this.env._t("New Word Document saved in Documents"),
                    sticky: false,
                });
        ...
        async _createPowerPoint() {
            ...
            const name =
                ...
                this.env.services.notification.notify({
                    type: "info",
                    message: this.env._t("New Powerpoint Document saved in Documents"),
                    sticky: false,
                });
                this.state["isOpen"] = false;
                setTimeout(function(){
        ...
        _createDocument(ev) {
            var self = this;
            var elems = document.querySelectorAll(".o-template-selected");

            [].forEach.call(elems, function(el) {
                var src = $(el).attr('src');
                if (src == 'apco/static/img/spreadsheet.svg'){
                    self._createSpreadsheet();
                }
                else if(src == 'apco/static/img/word.svg'){
                    self._createWord();
                }
                else if(src == 'apco/static/img/powerpoint.svg'){
                    self._createPowerPoint();
                }
            });
        }
        ...
        _Excel(ev) {
            ...
            [].forEach.call(elems, function(el) {
                el.classList.remove("o-template-selected");
            });
            $(ev.target).addClass('o-template-selected');
        }
    ...
    TemplateDialog.template = "apco.TemplateDialog";
