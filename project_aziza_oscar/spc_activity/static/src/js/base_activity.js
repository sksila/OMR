odoo.define('base_activity.base_activity', function (require) {
"use strict";
        ...
        this.events = _.extend(this.events || {}, {
            'click .link2event': function() {
                ...
                this._rpc({
                        ...
                        params: {
                            action_id: 'spc_activity.spc_mail_activity_action',
                        },
                    })
                    .then(function(r) {
                        r.res_id = self.eid;
            ...
            'click .link2showed': function() {
                var self = this;
                this._rpc({route: '/activity/notify_ack'}).always(function() {
                    self.destroy();
                });
            },
        });
...
WebClient.include({
    display_calendar_notif: function(notifications) {
        ...
        _.each(notifications, function(notif) {
            self.calendar_notif_timeouts[notif.activity_id] = setTimeout(function() {
                var notificationID = self.call('notification', 'notify', {
                    Notification: ActivityNotification,
                    title: notif.title,
                    message: notif.message,
                    eventID: notif.activity_id,
                });
                self.calendar_notif[notif.activity_id] = notificationID;
            }, notif.timer * 1000);
    ...
    get_next_calendar_notif: function() {
        session.rpc("/activity/notify", {}, {shadow: true})
    ...
    show_application: function() {
        ...
        this.call('bus_service', 'onNotification', this, function (notifications) {
            _.each(notifications, (function (notification) {
                if (notification[0][1] === 'activity.alarm') {
...
BasicModel.include({
     ...
     * @returns {Deferred}
     */
    _fetchSpecialParicipantStatus: function (record, fieldName) {
        var context = record.getContext({fieldName: fieldName});
        var attendeeIDs = record.data[fieldName] ? this.localData[record.data[fieldName]].res_ids : [];
        var activityID = _.isNumber(record.res_id) ? record.res_id : false;
        return this._rpc({
            model: 'res.partner',
            method: 'get_attendee_detail_activity',
            args: [attendeeIDs, activityID],
            context: context,
...
var Many2ManyParticipant = FieldMany2ManyTags.extend({
    // as this widget is model dependant (rpc on res.partner), use it in another
    // context probably won't work
    // supportedFieldTypes: ['many2many'],
    tag_template: "Many2ManyParticipantTag",
    specialData: "_fetchSpecialParicipantStatus",
...
field_registry.add('many2manyparticipant', Many2ManyParticipant);
