...
db_1_bank = models_10.execute_kw(db_1, uid_db1, password_db_1, 'oscar.bank', 'search_read', [[]],
                                  {'fields': ['id', 'code_bank','name', 'rib', 'type', 'email',
                                             'fax','adresse','phone','active']})
print('db_1_bank', db_1_bank)
total_count = 0
for record in db_1_bank:
    print('Bank---:', record)
    if record['type'] == 'tpe':
        search_bank = models_12.execute_kw(db_2, uid_db2, password_db_2, 'res.bank', 'search_read', [[['name','=',record['name']]]],
                                  {'fields': ['id']})
        if search_bank and search_bank[0]['id']:
            total_count += 1
            print(total_count)
            rib = record['rib']
            if not record['rib']:
                rib = '0'
            new_bank = models_12.execute_kw(db_2, uid_db2, password_db_2, 'res.partner.bank', 'create',
            [{'acc_number': rib, 'partner_id': 1, 'bank_id': search_bank[0]['id']}])
    # 'type': record['type'], v10 existe type: tpe ou banque, v12 n'existe pas
    # 'rib': record['rib'], bic ou rib en v12

logging.info("______________________END CRON JOBS__________________________")
print('****Total Created Bank***', total_count)